...
5. Run the following command to evaluate the vcf files and the according QC word report will be generated under the report folder prefixed with "Quartet_DNA_Report"
```
...
3. Run the following command to evaluate the RNA expresssion table and the according QC word report will be generated under the report folder named "Quartet_RNA_Report.docx"
```
...
### Protein QC
1. Create a new folder. e.g. ~/protein_qc with a subfolder for the QC report, e.g. ~/rna_qc/report
2. Put the Protein expresssion table, and metadata file to the new created folder
3. Run the following command to evaluate the Protein expresssion table and the according QC word report will be generated under the report folder named "Quartet_Protein_Report.docx"
```
docker run --rm -v ~/rna_qc:/data -it ghcr.io/chinese-quartet/quartet-omics-qc:latest protein-qc-report --exp-file /data/[exp-file].csv --meta-file /data/[phenotype-file].csv --output-dir /data/report
```
